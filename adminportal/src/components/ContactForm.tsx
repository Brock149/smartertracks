...
export default function ContactForm() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      const response = await fetch('https://formspree.io/f/xanykvbo', {
        method: 'POST',
        headers: {
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {status === 'error' && (
            <div className="bg-red-50 border border-red-200 rounded-lg p-4 text-red-700">
              Something went wrong. Please try again or email us directly at brockcoburn@smartertracks.com
            </div>
          )}
          ...
          <button
            ...
            className="w-full bg-blue-600 hover:bg-blue-700 disabled:bg-gray-400 text-white px-8 py-4 rounded-lg text-lg font-semibold transition-colors shadow-lg"
          >

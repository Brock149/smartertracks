-- First modify tool_transactions table to allow NULL to_user_id
ALTER TABLE tool_transactions
    ALTER COLUMN to_user_id DROP NOT NULL;

-- Remove any existing "Deleted User" entries
DELETE FROM auth.users WHERE email = 'deleted@internal';
...
BEGIN
    ...
    BEGIN
        ...
        -- For to_user_id, we can also set to NULL and store the name
        UPDATE tool_transactions
        SET 
            to_user_id = NULL,

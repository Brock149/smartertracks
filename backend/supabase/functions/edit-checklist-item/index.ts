...
serve(async (req) => {
  ...
  try {
    ...
    const { id, item_name, required } = await req.json()

    if (!id || !item_name || typeof required !== 'boolean') {
      return new Response(JSON.stringify({ error: 'id, item_name, and required are required' }), {
